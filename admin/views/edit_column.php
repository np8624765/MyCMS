...
$(document).ready(function(){
	...
	function showResponse(responseText, statusText){  
   		  if($.trim(responseText)=="ok") {
			  ...
			  $(".message").find("span").text("编辑失败！图片上传失败");
		  }else{
			  $(".message").addClass("red");
...
<div class="box">
  ...
  <table class="wrap" cellspacing="0">
    <tr class="top">
      <td class="top_l"></td>
      <td class="top_m"><div class="title"><span>编辑栏目</span></div></td>
      <td class="top_r"></td>
    </tr>
    <tr class="mid">
      <td class="mid_l"></td>
      ...
      <div class="content">
      	  <div class="intro">
          	<p>当前位置：<a href="index.php?action=column_manage&method=show_column">栏目管理</a>->编辑栏目</p>
            <img src="views/images/edit_column.png" />
            <span>在这里，您可以编辑已有栏目！<br /><br />特别注意：请在修改为介绍型栏目或链接型栏目之后，到文章管理模块为该栏目添加一篇子文章作为内容，否则栏目没有效果。</span>
          </div>
       <form id="myform" action="index.php?action=column_manage&method=update_column&cid=<?php echo $col['cid'];?>" method="post" enctype="multipart/form-data">
 	  	<table class="setting" border="0" cellspacing="0">
      	<thead><tr><th>栏目设置列表</th><th></th><th></th></tr></thead>
        <tbody>
            <tr>
                <td width="15%">栏目名称：</td>
                <td width="30%"><input type="text" name="col_name" size="20" value="<?php echo $col['cname'];?>"/></td>
            ...
            <tr>
                ...
                <td>
                	<select name="col_fid" id="col_fid">
                        ...
                        <?php foreach($main as $value){ if($value['cid']!=$col['cid']){?>
                        <option value="<?php echo $value['cid']?>" <?php if($value['cid']==$col['fid']){echo "selected='selected'";};?>><?php echo $value['cname']?></option>
                        <?php }}?>
                    </select>
                </td>
            ...
            <tr>
                <td>显示优先级：</td>
                <td><input type="text" name="col_sid" size="10" value="<?php echo $col['sid'];?>"/></td>
                <td>设置该栏目显示时的排列顺序，优先级最高为1，级数越大优先级越低，不填写即默认，优先级最低，排列在最后。</td>
            </tr>
             <tr>
                <td>栏目关键字：</td>
                <td><input type="text" name="col_key" size="40" value="<?php echo $col['ckey'];?>"/></td>
                <td>设置栏目关键字利于被搜索引擎收录，多个关键字请以“，”隔开</td>
            </tr>
             <tr>
                <td>栏目描述：</td>
                <td><input type="text" name="col_des" size="40" value="<?php echo $col['cdes'];?>"/></td>
            ...
            <tr>
             ...
             <tr>
                <td>栏目类型：</td>
                <td>
                导航型<input type="radio" name="col_type" value="0" <?php if($col['ctype']==0){echo "checked='checked'";};?>/>
                专题型<input type="radio" name="col_type" value="1" <?php if($col['ctype']==1){echo "checked='checked'";};?>/>
                </td>
             ...
             <tr>
                <td>栏目内容形式：</td>
                <td>
                列表型<input type="radio" name="col_con" value="0" <?php if($col['ccontent']==0){echo "checked='checked'";};?>/>
                介绍型<input type="radio" name="col_con" value="1" <?php if($col['ccontent']==1){echo "checked='checked'";};?>/>
                链接型<input type="radio" name="col_con" value="2" <?php if($col['ccontent']==2){echo "checked='checked'";};?>/>
                </td>
                <td><span class="must">*</span>列表型栏目内容是文章列表，介绍型栏目内容是一篇介绍性文章，链接型栏目点击后跳转到其他网页。</td>
            </tr>
            <tr id="url">
                <td>链接型栏目URL：</td>
                <td><input type="text" name="col_curl" size="40" value="<?php echo $col['curl'];?>"/></td>
